...
package org.lionart.mx.controls
{
	 ...
	 *  @productversion Flex 3
	 */
	public class ArabicDateChooser extends UIComponent implements IFocusManagerComponent, IFontContextComponent
	{
		 ...
		 *  @productversion Flex 3
		 */
		public function ArabicDateChooser()
		{
			super();
			...
			// This line was added to support style name.
			styleName = "arabicDateChooser";
		}
		 ...
		 */
		mx_internal var dateGrid:ArabicCalendarLayout;
		...
		{
			if (ArabicDateChooser.createAccessibilityImplementation != null)
				ArabicDateChooser.createAccessibilityImplementation(this);
		}
		...
		override protected function createChildren():void
		{
			...
			if (!dateGrid)
			{
				dateGrid = new ArabicCalendarLayout();
		...
		private function upYearButton_buttonDownHandler(event:Event):void
		{
			...
			{
				var newDate:Object = ArabicCalendarLayout.getNewIncrementDate(displayedYear, displayedMonth, 1, 0);
		...
		private function fwdMonthButton_buttonDownHandler(event:Event):void
		{
			...
			if ((selectableRange) &&
				(dateGrid.selRangeMode == 1 || dateGrid.selRangeMode == 3))
			{
				var newDate:Object = ArabicCalendarLayout.getNewIncrementDate(displayedYear, displayedMonth, 0, 1);
				var testDate:Date = new Date(newDate.year, newDate.month,
					selectableRange.rangeEnd.getDate());
		...
		{
			_selectedDate = ArabicCalendarLayout(event.target).selectedDate;
