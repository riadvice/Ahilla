...
package org.lionart.arabic.calendar
{
    ...
    public class HijriCalendar extends Calendar
    {
        ...
        /* Constuctor                                  */
        /*---------------------------------------------*/
        public function HijriCalendar(dateTime : Date = null, year:* = null, month:* = null, 
            days:*=null, hours:*=null,minutes:*=null,seconds:*=null,ms:*=null) : void
        {
            super( this );
            adjustemnt = 0x0100;
            if ( dateTime )
            {
                _internalDate = dateTime;
            }
            ...
            {
                _internalDate = new Date(-42522544800000);
                var totalDays : int = days;
                var iterator : int = 0;
                totalDays += year*354;
                for ( iterator=1; iterator <= month; iterator++ )
                {
                    totalDays+=getDaysInMonthInEra(year, iterator, hijriEra);
                }
                _internalDate.hours = hours;
                _internalDate.minutes = minutes;
                _internalDate.seconds = seconds;
                _internalDate.milliseconds = ms;
                _internalDate.date += totalDays;
                if ( days == null || days == 0 )
                {
                    days = 1;
                }
                roundApproximation(year, month, days);
            }
        }

        protected function roundApproximation(year:* = null, month:* = null,days:*=null) : void
        {
            var approximationSign : int = year <= getYear() ? -1 : 1;
            while ( year != getYear())
            {
                _internalDate.date += 100 * approximationSign;
            }
            ...
            {
                _internalDate.date += 1 * approximationSign;
            }
        }
        ...
        public function get hijriAdjustment() : int
        {
            ...
            {
                // Must find a way to do it. Because we can not acces windows regisrty.
        ...
        /**
         * Pull apart a Date value into year, month, and day.
         */
         ...
         */
        override public function addMonths( months : int, time : Date = null ) : Date
        {
            time = verifyTime( time );
            pullDateApart( time );
            if ( months > 0 )
        ...
        override public function addYears( years : int, time : Date = null ) : Date
        {
            time = verifyTime( time );
            return addMonths( years * 12, time );
        }

        /**
         * Extract the components from a DateTime value.
         */
        override public function getDayOfMonth( time : Date = null ) : int
        {
            pullDateApart( time );
            return _dayRef;
        }

        override public function getDayOfWeek( time : Date = null ) : int
        {
            time = verifyTime( time );
            return DateTimeUtils.dayOfWeek( time );
        }

        override public function getDayOfYear( time : Date = null ) : int
        {
            pullDateApart( time );
         ...
         */
        override public function getEra( time : Date = null ) : int
        {
            return hijriEra;
        }
         ...
         */
        override public function toDateTimeInEra( year : int, month : int, day : int, hour : int, minute : int, second : int,
